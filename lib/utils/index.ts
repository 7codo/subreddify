...
export function getPlanFeaturesById(variantId: number) {
  switch (variantId) {
    case VARIANT_ID.free.monthly:
    case VARIANT_ID.free.yearly:
...
export function filterPlansByInterval(
  ...
  const updatedPlans = plans.filter((plan) => plan.interval === interval);

  const sortedPlans = updatedPlans.sort((a, b) => {
    // Place free plans first
    ...
    // Secondary sort: monthly before yearly
    const aIsMonthly = a.interval === "month"; // Fixed comparison operator
    const bIsMonthly = b.interval === "month"; // Fixed comparison operator
